  ...
  var vm = window.vm = new Vue({
    ...
    methods: {
      ...
      getDictList: function () {
        return vm.$http.get('/sys/dict/type/all').then(function (res) {
          if (res.data.code !== 0) {
            return
          }
          window.dictList = res.data.data
        }).catch(function () {});
      },
      // 获取菜单列表
  ...
  return {
    template: '\
      <nav class="aui-navbar" :class="\'aui-navbar--\' + navbarLayoutType">\
          ...
          <el-menu class="aui-navbar__menu" mode="horizontal">\
            ...
            <el-menu-item index="3" v-if="$hasPermission(\'sys:notice:all\')">\
              <el-badge :is-dot="messageTip">\
                <a href="#notice-notice-user"><i class="el-icon-bell"></i></a>\
              </el-badge>\
            </el-menu-item>\
        ...
        <!-- 弹窗, 修改密码 -->\
        <update-password v-if="updatePasswordVisible" ref="updatePassword" :user="user"></update-password>\
      </nav>\
    ',
    data: function () {
      return {
    ...
    created () {
      ...
      if (window.location.protocol === 'http:') {
        socketURL = 'ws://' + window.location.host + contextPath + '/websocket';
      } else {
        socketURL = 'wss://' + window.location.host + contextPath + '/websocket';
      }
    ...
    methods: {
      getUnReadCount () {
        self.$http.get('/sys/notice/mynotice/unread').then(function (res) {
          if (res.data.code !== 0) {
            return self.$message.error(res.data.msg);
          }
          if (res.data.data > 0) {
            self.messageTip = true
          }
        }).catch(function () {});
      },
      // 刷新
      ...
      updatePasswordHandle: function () {
        self.updatePasswordVisible = true;
        self.$nextTick(function () {
          self.$refs.updatePassword.init();
        });
      },
      // 退出
      ...
      dataRule: function () {
        var validateConfirmPassword = function (rule, value, callback) {
          if (self.dataForm.newPassword !== value) {
            return callback(new Error(self.$t('updatePassword.validate.confirmPassword')));
          }
          callback();
        }
        return {
