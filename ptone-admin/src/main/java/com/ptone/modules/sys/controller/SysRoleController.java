...
public class SysRoleController {
	...
	public Result<SysRoleDTO> get(@PathVariable("id") Long id){
		...
		//查询角色对应的菜单
		List<Long> columnIdList = sysRoleColumnService.getColumnIdList(id);
		data.setColumnIdList(columnIdList);
		//查询角色对应的数据权限
		List<Long> deptIdList = sysRoleDataScopeService.getDeptIdList(id);
		data.setDeptIdList(deptIdList);

		List<SysUserDTO> userList = sysUserService.getListByRoleId(id);
